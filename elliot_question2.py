...
df_avgRain = df_usa.groupby(year_col)[[rainfall_col]].mean().reset_index()
df_avgSea = df_usa.groupby(year_col)[[sea_level_col]].mean().reset_index()

#Define 3 seperate plots
fig, axes = plt.subplots(1, 3, figsize=(20, 5))

#Rainfall plot
axes[0].plot(df_avgRain[year_col], df_avgRain[rainfall_col], marker='o', linestyle='-', color='blue')
...
#Sea level plot
axes[1].plot(df_avgRain[year_col], df_avgSea[sea_level_col], marker='s', linestyle='--', color='red')
...
axes[2].plot(df_avgRain[year_col], df_avgRain[rainfall_col], marker='o', linestyle='-', color='blue', label="Avg Rainfall (mm)")
...
axes3 = axes[2].twinx()
axes3.plot(df_avgRain[year_col], df_avgSea[sea_level_col], marker='s', linestyle='--', color='red', label="Avg Sea Level Rise (mm)")
